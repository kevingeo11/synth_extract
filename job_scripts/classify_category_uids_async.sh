...
#SBATCH --job-name=classify-category-uids
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
usage() {
    cat <<'EOF'
Usage:
  sbatch job_scripts/classify_category_uids_async.sh MANIFEST_FILE [MANIFEST_INDEX]

Examples:
  sbatch job_scripts/classify_category_uids_async.sh data/uid_manifest/category_uids_0001.txt
  sbatch job_scripts/classify_category_uids_async.sh data/uid_manifest/category_uids_0001.txt 1
...
The Python worker always updates polymer_material_papers.category_class.
EOF
}
...
VLLM_LOG="$LOG_DIR/vllm-category-${SLURM_JOB_ID:-manual}.log"
...
for required_path in \
    ...
    "$FULLTEXT_ROOT" \
    "$UID_FILE" \
    "$PROJECT_DIR/scripts/classify_category_uids_async.py"; do
...
python scripts/classify_category_uids_async.py "${CLASSIFIER_ARGS[@]}"

echo "Category classification job completed successfully"
