#!/usr/bin/env python3
"""Convert Springer Nature XML files in a tracking database to Markdown.
...
-----
    python scripts/process/springer_xml_to_md.py \
        --db data/process/springer_nature_track.db \
...
LOG = logging.getLogger("springer_xml_to_md")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Convert pending Springer Nature XML files from a tracking "
            "SQLite database to Markdown and persist progress in convert_md."
        )
    )
    parser.add_argument(
        "--db",
        ...
        for row in pending_rows:
            ...
            if (
                ...
                if conversion_function is None:
                    from synth_extract.mining.process.springer_xml import (
                        springer_xml_to_markdown,
                    )

                    conversion_function = springer_xml_to_markdown
