#!/usr/bin/env python3
"""Convert Europe PMC XML files in a tracking database to Markdown.

Pending rows are those whose ``convert_md`` value is NULL. For each pending
row, the XML file is expected at::

    FULLTEXT_ROOT / CANONICAL_SOURCE / PAPER_UID / (PAPER_UID + ".xml")

The Markdown file is written beside the XML. Successful conversions and
already-existing non-empty Markdown files are recorded as ``convert_md = 1``.
Missing XML files and conversion failures remain NULL so a later run can
retry them.

Usage
-----
    python scripts/process/europepmc_xml_to_md.py \
        --db data/process/europepmc_track.db \
...
LOG = logging.getLogger("europepmc_xml_to_md")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Convert pending Europe PMC XML files from a tracking SQLite "
            "database to Markdown and persist progress in convert_md."
        )
    )
    parser.add_argument(
        "--db",
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory containing source/paper_uid/paper_uid.xml paths.",
    )
    parser.add_argument(
        "--commit-every",
        type=int,
...
def convert_tracking_database(
    ...
    with closing(sqlite3.connect(db_path, timeout=60)) as conn:
        ...
        for row in pending_rows:
            ...
            xml_path = paper_dir / f"{paper_uid}.xml"
            ...
            if (
                ...
                summary.missing_xml += 1
                LOG.warning("XML file not found: %s", xml_path)
                continue
            else:
                if conversion_function is None:
                    from synth_extract.mining.process.europepmc_xml import (
                        europepmc_xml_to_markdown,
                    )

                    conversion_function = europepmc_xml_to_markdown
                ...
                try:
                    output_path = conversion_function(xml_path)
                except Exception:
                    summary.failed += 1
                    LOG.exception(
                        "Conversion failed: %s",
...
def main(argv: list[str] | None = None) -> int:
    ...
    LOG.info(
        ...
        "missing_xml=%d invalid_row=%d failed=%d remaining=%d",
        summary.selected,
        summary.converted,
