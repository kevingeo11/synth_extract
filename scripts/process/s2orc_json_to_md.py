#!/usr/bin/env python3
"""Convert S2ORC JSON files in a tracking database to Markdown.

Pending rows are those whose ``convert_md`` value is NULL. For each pending
row, the JSON file is expected at::

    FULLTEXT_ROOT / CANONICAL_SOURCE / PAPER_UID / (PAPER_UID + ".json")

The Markdown file is written beside the JSON file. Successful conversions and
already-existing non-empty Markdown files are recorded as ``convert_md = 1``.
Missing JSON files and conversion failures remain NULL so a later run can
retry them.

Usage
-----
    python scripts/process/s2orc_json_to_md.py \
        --db data/process/s2orc_track.db \
...
LOG = logging.getLogger("s2orc_json_to_md")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Convert pending S2ORC JSON files from a tracking SQLite "
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory containing source/paper_uid/paper_uid.json paths.",
    )
    parser.add_argument(
        "--commit-every",
        type=int,
...
def convert_tracking_database(
    ...
    with closing(sqlite3.connect(db_path, timeout=60)) as conn:
        ...
        for row in pending_rows:
            ...
            json_path = paper_dir / f"{paper_uid}.json"
            ...
            if (
                ...
                LOG.warning("JSON file not found: %s", json_path)
                continue
            else:
                if conversion_function is None:
                    from synth_extract.mining.process.s2orc_json import (
                        s2orc_json_to_markdown,
                    )

                    conversion_function = s2orc_json_to_markdown
                ...
                try:
                    output_path = conversion_function(json_path)
                except Exception:
                    summary.failed += 1
                    LOG.exception(
                        "Conversion failed: %s",
...
def main(argv: list[str] | None = None) -> int:
    ...
    LOG.info(
        ...
        "missing_json=%d invalid_row=%d failed=%d remaining=%d",
        summary.selected,
        summary.converted,
