...
class PaperClassifier(ABC):
    ...
    def response_format() -> dict[str, Any]:
        """Return the decision JSON schema required from the model endpoint.

        Completion metadata is read from the provider response and must not be
                ...
                "schema": {
                    ...
                    "properties": {
                        "label": {
                            "type": "boolean",
                            "description": (
                                "Whether the paper matches the classification "
                                "criteria."
                            ),
                        },
                        "category": {
                            ...
                            "description": (
                                "The classification category, when one is "
                                "available."
                            ),
                        },
                    },
    ...
    @staticmethod
    def _completion_metadata(completion: Any) -> CompletionMetadata:
        """Normalize useful fields from a raw ChatCompletion response."""
    ...
    @classmethod
    def _parse_completion(cls, completion: Any) -> ClassificationOutcome:
