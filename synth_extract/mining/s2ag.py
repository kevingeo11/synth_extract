...
def build_authors_db(
    dump_dir: str,
    db_path: str,
    scratch_dir: str | None = None,
    batch_size: int = 200_000,
    skip_missing_name: bool = False,
    log_every_seconds: float = 60.0,
    cache_size_kib: int = 2_000_000,   # ~2GB
    mmap_size_bytes: int = 4_000_000_000,  # ~4GB
) -> None:
    """
    Create a SQLite database containing authorid -> author metadata mappings
    from a directory of gzipped JSONL Semantic Scholar authors dump files.

    Same build strategy as build_abstracts_db / build_papers_db:
      * Always a fresh build; raises if db_path already exists.
      * Loads into a plain (unindexed) rowid table first -- pure appends,
        no B-tree lookups on authorid during the hot loop -- then builds a
        UNIQUE index on authorid afterwards in one efficient sorted pass.
      * Optionally builds the DB on fast local scratch storage and copies
        the finished file to db_path at the end.
      * Logs on a wall-clock heartbeat (every log_every_seconds) in
        addition to per-file boundaries.
    ...
    Columns map 1:1 onto the top-level keys of each JSON record. Nested
    fields (externalids, aliases, affiliations) are stored verbatim as
    JSON-serialized TEXT -- use json.loads(...) on them when reading rows
    back.
    ...
    batch_size : int
        Rows accumulated before each executemany/commit.
    skip_missing_name : bool
        If True, rows with null/empty name are skipped entirely.
    log_every_seconds : float
        Minimum wall-clock time between progress log lines.
    cache_size_kib : int
        SQLite page cache size in KiB (passed as a negative value to PRAGMA cache_size).
    ...
    logger.info(
        f"Config: batch_size={batch_size:,} "
        f"skip_missing_name={skip_missing_name} "
        f"log_every_seconds={log_every_seconds} "
        f"cache_size_kib={cache_size_kib:,} mmap_size_bytes={mmap_size_bytes:,}"
    )
    ...
    # NOTE: no PRIMARY KEY / index on authorid yet. Plain rowid table so
    # every insert is a pure append -- no B-tree lookups on authorid.
    cur.execute(
        """
        CREATE TABLE IF NOT EXISTS authors_raw (
            authorid INTEGER,
            externalids TEXT,
            url TEXT,
            name TEXT,
        ...
        for file_idx, path in enumerate(gz_files, start=1):
            ...
            with gzip.open(path, "rt", encoding="utf-8") as f:
                for line in f:
                    ...
                    name = rec.get("name")
                    if skip_missing_name and not name:
                        total_skipped += 1
                        continue
                    ...
                    row = (
                        ...
                        json.dumps(affiliations) if affiliations is not None else None,
                        rec.get("homepage"),
                        rec.get("papercount"),
                        rec.get("citationcount"),
                        rec.get("hindex"),
    ...
    logger.info(
        f"Insert phase done. Inserted {total_rows:,} rows "
        f"(skipped {total_skipped:,} missing-name records, "
        f"{total_bad_lines:,} malformed/missing-authorid lines) in {elapsed:,.0f}s "
        f"({total_rows / elapsed:,.0f} rows/sec avg)."
    )
    ...
    # Now build the real, indexed table in one efficient sorted pass,
    # instead of having maintained the index incrementally during load.
    logger.info("Building final indexed table (authorid PRIMARY KEY)...")
    t0 = time.time()
    cur.execute(
        """
    ...
    conn.commit()
    cur.execute("DROP TABLE authors_raw;")
    conn.commit()
    logger.info(f"Final table built in {time.time() - t0:,.0f}s")
...
if __name__ == "__main__":
    ...
    dump_dir = "data/s2ag/authors"
    db_path = "data/s2ag/authors.db"

    build_authors_db(dump_dir=dump_dir, db_path=db_path, scratch_dir=None)